...
class ScoreEvent(object):
    """
    An abstract class representing a score event (note or chord)
    """
    ...
    @property
    def polyphony(self):
        return len(self.underlying_events())

    @abstractmethod
    def underlying_events(self):
        """
        Returns a list of finer resolution events contained in the ScoreEvent.
        E.g., a chord consists of several Notes
        """
        pass
...
class Chord(ScoreEvent):
    """
    Represents a chord consisting of multiple notes
    """

    def __init__(self, notes, **kwargs):
        """
        Parameters:
            notes (list of Notes): list of notes to put in the chord container
                Provide empty list if initializing with unknown notes
        """
    ...
    def del_note(self, pname, octave):
        note = Note(pname, octave)
